...
# 가로수의 위치를 나타내는 정수는 1,000,000,000 이하이다. 가로수의 위치를 나타내는 정수는 모두 다르다.
...
min_distance = tree[n-1]
for i in range(n-2):
    min_distance = min(min_distance, math.gcd(tree[i+2]-tree[i+1], tree[i+1]-tree[i]))



length = tree[n-1] - tree[0]
print(length // min_distance - n + 1 if length % 2 == 0 else length // min_distance - n)