...
# 오박사 : 나는 태초마을의 포켓몬 박사 오민식 박사라네. 다솜아, 포켓몬을 잡을 때는, 
# 일단 상대 포켓몬의 체력을 적당히 바닥으로 만들어놓고 몬스터 볼을 던져야 한단다. 
# 자, 내 포켓몬 이상해꽃으로 한번 잡아보렴. 포켓몬의 기술을 쓰는 것을 보고 포켓몬을 줄지 안줄지 결정을 하겠네. 
# 자 한번 해보아라. 다솜아.
...
# 하지만 상록숲 체육관의 리더는 실종된 상태. 따라서 회색마을부터 도전하기로 했다. 
# 체육관의 리더를 이기면서, 로켓단을 해체시키기도 하고, 여러 가지 사건도 있었다. 
# 결국 전설의 포켓몬도 잡고, 이제 사천왕을 이기려고 도전하기로 했다. 사천왕은 모두 가볍게 이기고, 
# 이제 마지막 라이벌 오!영!식! 이다.
...
a = {}
for i in range(n):
    a[str(i+1)] = sys.stdin.readline().rstrip()
b = {v:k for k, v in a.items()}
for _ in range(m):
    qeustion = sys.stdin.readline().rstrip()
    if qeustion in a.keys():
        print(a[qeustion])
    else:
        print(b[qeustion])