...
# 빵집이 있는 곳은 R*C 격자로 표현할 수 있다. 첫째 열은 근처 빵집의 가스관이고, 
# 마지막 열은 원웅이의 빵집이다.
...
# 마지막 열에서 끝나야 한다. 각 칸은 오른쪽, 오른쪽 위 대각선, 오른쪽 아래 대각선으로 연결할 수 있고, 
# 각 칸의 중심끼리 연결하는 것이다.
...
def dfs(x, y):
    global result
    if x == -1 or x == r: # 범위를 벗어나면 즉시 종료
        return False

    if stage[x][y] != '.': #갈 수 없어도 즉시 종료
        return False
...
move = [[-1, 1], [0, 1], [1, 1]]
for _ in range(r):
    stage.append(list(input().rstrip()))
